	 ...
	 CREATE OR REPLACE FUNCTION multiplydata(offsetNum integer,limitNum integer,iteration_num integer,tableName varchar, out result integer)  AS $Body$
      ...
      BEGIN
        ...
        IF tableName='contact' then
           ...
           sql:='insert into contact (name,sfid,'||newcolumns|| ')  select (name||'' ''||'||iteration_num||') as name, ('||iteration_num||'||substring(sfid from 3)) as sfid,'|| newcolumns||' from contact order by id asc  offset '||offsetNum||' limit '||limitNum;
           EXECUTE  sql; 
        else
           newcolumns:=(select string_agg(column_name,',') from information_schema.columns
           where table_schema='public' and table_name=tableName and column_name not in ('id','sfid'));
           sql:='insert into '||tableName||' (sfid,'||newcolumns|| ')  select ('||iteration_num||'||substring(sfid from 3)) as sfid,'|| newcolumns||' from '||tableName||' order by id asc  offset '||offsetNum||' limit '||limitNum;
           EXECUTE  sql; 
        end IF;
