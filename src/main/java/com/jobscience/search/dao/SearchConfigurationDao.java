...
public class SearchConfigurationDao {
    ...
    public List getSearchConfig() {
        List<Map> result = daoRwHelper.executeQuery(daoRwHelper.datasourceManager.newSysRunner(),
                "select val_text from config where name = ? and org_id is null", COL_NAME);
        return result;
    }

    public void resetSearchConfig() {
        daoRwHelper.executeUpdate(daoRwHelper.datasourceManager.newSysRunner(),
    ...
    public void saveSearchConfig(String content) {
        List list = daoRwHelper.executeQuery(daoRwHelper.datasourceManager.newSysRunner(),
                "select 1 from config where name = ? and org_id is null ", COL_NAME);
        if (list.size() == 0) {
            daoRwHelper.create(daoRwHelper.datasourceManager.newSysRunner(),
